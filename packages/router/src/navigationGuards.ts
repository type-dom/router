...
export function extractComponentsGuards(
  ...
  for (const record of matched) {
    ...
    for (const name in record.components) {
      const rawComponent: RawRouteComponent | (() => any) =
      ...
      if (isRouteComponent(rawComponent)) {
        ...
        // console.warn('isRouteComponent . ') // todo
        const options = rawComponent;
        // (rawComponent as any).__vccOpts || rawComponent
        const guard = (options as any)[guardType];
        if (guard) {
          guards.push(
        ...
        const componentPromise: Promise<
        ...
        guards.push(() =>
          componentPromise.then((resolved) => {
            ...
            const resolvedComponent = isESModule(resolved)
              ? getClassFromModule(resolved) // resolved.default
              : resolved;
            ...
            const options = resolvedComponent;
            const guard = (options as any)[guardType];
...
export function getClassFromModule(module: RouteComponent | Module) {
  for (const value of Object.values(module)) {
    if (
      ...
      return value as typeof TypeElement;
    }
  }
  return;
}
          ...
          (record) =>
            ...
            Promise.all(
              Object.keys(record.components).reduce((promises, name) => {
                ...
                // console.warn('loadRouteLocation . name rawComponent is ', name, rawComponent);
                if (
                  typeof rawComponent === 'function' &&
                  ...
                  promises.push(
                    (rawComponent as Lazy<RouteComponent>)().then(
                      (resolved) => {
                        ...
                        // console.warn('resolve is ', resolved);
                        const resolvedComponent = isESModule(resolved)
                          ? getClassFromModule(resolved) // resolved.default
                          : resolved;
                        // keep the resolved module for plugins like data loaders
